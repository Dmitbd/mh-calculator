...
test("renders the antique calculator instructions with its domain rules", () => {
  ...
  expect(
    screen.getByText(
      "Нормализованные монеты, собственные карты, распределение монет и настройка кешбэка автоматически сохраняются и восстанавливаются между посещениями. Кнопка «Сбросить расчёт» очищает сохранённые значения и включает кешбэк.",
    ),
  ).toBeTruthy();
});
