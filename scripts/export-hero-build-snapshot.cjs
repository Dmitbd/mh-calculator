...
const { requestBootstrap } = require("../src/shared/lib/dataBootstrap.ts");
const { loadRemoteHeroBuildSnapshot } = require("../src/features/builds/data/heroBuildSnapshotRemote.ts");
const { createHeroBuildSnapshot, parseHeroBuildSnapshot } = require("../src/features/builds/data/heroBuildSnapshot.ts");
...
async function main() {
  if (process.argv.includes("--from-bundled")) {
    ...
    const files = createHeroBuildSnapshot({
      ...
      heroBuilds: Object.entries(heroBuilds).map(([heroId, buildSet]) => ({ heroId, buildSet })),
    });
  ...
  const { files, parsed } = await loadRemoteHeroBuildSnapshot({ config, manifest });
  validateAssets(parsed.heroBuilds);
  replaceSnapshotDirectory(files);
}
...
function replaceSnapshotDirectory(files) {
  const parent = path.join(root, "src/features/game-data/snapshots");
  const target = path.join(parent, "hero-builds");
  const temporary = fs.mkdtempSync(path.join(parent, ".hero-builds-"));
  const backup = `${target}.previous`;
  try {
    fs.writeFileSync(path.join(temporary, "hero-builds.json"), files.resourceJson, { flag: "wx", mode: 0o644 });
    fs.writeFileSync(path.join(temporary, "manifest.json"), files.manifestJson, { flag: "wx", mode: 0o644 });
