...
> Обновлено: в форму добавлены режим игры, оружие, руна и пробуждение оружия —
> все они обязательны. Сообщения об ошибках выводятся пользователю на русском.

Перед скачиванием проверить:

- режим игры выбран (`pvp` или `pve`);
- имя героя заполнено;
- выбрано оружие (`Оружие`) и оно существует в `equipment/artifacts.json`;
- выбрана руна (`Руна`) и она существует в `equipment/runes.json`;
- все слоты пробуждения оружия заполнены валидными цветами;
- выбраны все 3 ветки;
...
Все сообщения об ошибках валидации — на русском языке и показываются
рядом с кнопкой `Скачать JSON`.
...
- [x] Создать `downloadJson.ts`.
- [x] Создать `DownloadJsonButton.tsx`.
- [x] При клике собрать export JSON.
- [x] Запустить validation.
- [x] Если есть errors, показать список и не скачивать.
- [x] Если errors нет, скачать `{slugified-hero-name}.json`.
