...
Системная шапка Expo Router для обоих маршрутов отключена. Каталог и экран героя используют общий [ScreenHeader](../src/shared/ui/ScreenHeader.tsx): заголовок имеет единый размер `30px`, занимает не более двух центрированных строк, а геометрический шеврон всегда остаётся в вертикальном центре шапки. Навигация назад использует историю, а при её отсутствии возвращается в `/` или `/heroes`.
...
- дерево использует круглые оригинальные APK-иконки вместо квадратных карточек: левая ветка светится синим, центральная зелёным, правая фиолетовым, горизонтальный уровень ветвления является одной линией с двумя цветовыми половинами и игровыми ромбами позади непрозрачных круглых контейнеров, поэтому не пересекает видимые иконки при любом масштабе; неактивная нода тонируется отдельным слоем поверх изображения без изменения его opacity, а каждый вертикальный соединитель имеет единое состояние по всей длине и наследует его от предыдущей ноды;
- под каждой малой нодой постоянно показана компактная подпись с названием и значением характеристики, под большой — только имя выбранного скилла без уровня и длинного описания; соединительные ромбы не имеют видимой подписи, а их точная семантика остаётся доступной screen reader;
