...
LKG хранится через AsyncStorage неизменяемыми поколениями. Запись сохраняет resource и manifest, перечитывает и полностью проверяет пару, а затем последней малой записью создаёт `:committed` marker. На каждом чтении storage перечисляет не более `512` ключей и `32` поколений, считает кандидатами только поколения с marker, полностью проверяет их пары и выбирает максимум по total order `(contentUpdatedAt, contentVersion, checksum)`; pointer является только подсказкой и восстанавливается на найденный максимум. Поэтому reader никогда не принимает или не удаляет staging активного writer, а повреждённый/stale pointer и гонка независимых JS contexts не понижают наблюдаемый snapshot, несмотря на отсутствие межпроцессного CAS в AsyncStorage. После успешной проверки сохраняются четыре наиболее свежих committed поколения, а повреждённые committed и более старые committed удаляются при наличии `removeItem`; unmarked staging остаётся нетронутым. Переполнение budget или ошибка enumeration закрываются безопасным переходом к bundled snapshot. После marker writer повторно сканирует максимум и только затем чинит pointer.
