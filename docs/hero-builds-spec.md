...
На каждом чтении из всех ключей выбираются только exact generation keys; посторонние ключи не входят в budget. Более `32` поколений закрывают чтение безопасным переходом к bundled snapshot. Каждый blob полностью проверяется, максимум выбирается по total order `(contentUpdatedAt, contentVersion, checksum)`, а pointer используется только как порядок чтения существующего generation key и восстанавливается на найденный максимум. Поэтому corrupt/stale pointer и гонка независимых JS contexts не понижают наблюдаемый snapshot, несмотря на отсутствие межпроцессного CAS в AsyncStorage. После проверки сохраняются четыре наиболее свежих валидных поколения, а повреждённые complete blobs и более старые поколения удаляются при наличии `removeItem`.
