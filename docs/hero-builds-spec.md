...
На уровне Supabase RLS все пользователи могут читать только `published`-строки. JWT с `app_metadata.role = admin` может читать также `draft` и выполнять insert/update/delete; одного статуса `authenticated` недостаточно.
