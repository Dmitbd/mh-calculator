...
На уровне Supabase RLS все пользователи могут читать только `published`-строки. JWT с `app_metadata.role = admin` может читать также `draft` и выполнять разрешённые lifecycle-операции; одного статуса `authenticated` недостаточно. На каждого `hero_id` хранится не более одной строки: `draft` атомарно становится `published`, а опубликованную строку нельзя удалить, перевести обратно в `draft` или перенести на другой `hero_id`. Payload опубликованной строки обновляется отдельной repository-операцией без смены статуса.
