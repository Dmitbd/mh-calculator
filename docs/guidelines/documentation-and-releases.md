...
- Текущий baseline `2026-08-20` для static production export без Supabase env после обязательной очистки Metro cache на закреплённых Node/npm: `1 987 844` raw bytes и `482 933` gzip-9 bytes. Перед обновлением повторяющиеся данные трёх событий соперничества были нормализованы в общий каталог ресурсов, общий снимок шкалы и компактные кортежи наград; это уменьшило raw bundle на `85 002` bytes относительно первой реализации. Новый raw ceiling `2 020 000` фиксирует измеренную стоимость трёх самостоятельных экранов после этой оптимизации, а gzip ceiling не повышается и остаётся `490 000`.
