...
- сопоставить результаты с ceilings `1 950 000` raw / `490 000` gzip-9 bytes и operational E2E; если измерения покажут регрессию, скорректировать критический набор, timeout или asset format без all-catalog preload;
...
Текущий каталог содержит только несколько build-ready карточек в bundled/тестовых сценариях, а browser E2E не показал measured render bottleneck. Поэтому виртуализация списка намеренно не добавлена без runtime evidence; решение пересматривается по результатам production/native измерений выше.
