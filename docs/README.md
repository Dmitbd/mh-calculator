...
- Данные: один ограниченный `/bootstrap` подтверждает backend и совместимость атомарного manifest; каталог и detail читают только один полный опубликованный resource, который проходит общий ограниченный runtime-парсер и выбирается в порядке `remote → last-known-good → bundled snapshot`. LKG определяется сканированием ограниченного набора полностью проверенных immutable-поколений по стабильному server order, а pointer остаётся восстанавливаемой подсказкой.
