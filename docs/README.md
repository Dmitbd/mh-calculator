...
- Данные: один ограниченный `/bootstrap` подтверждает backend и совместимость атомарного manifest; полный опубликованный resource проходит общий ограниченный runtime-парсер и выбирается в порядке `remote → last-known-good → bundled snapshot`, а проверенный remote сохраняется логически атомарным поколением.
