...
- Загрузка: каталог и билд не показывают bundled данные до завершения выбора remote/fallback; запрос каталога ограничен `8` секундами, а фоновые обновления сохраняют видимый контент.
