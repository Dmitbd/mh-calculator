...
- Загрузка: каталог и билд не показывают bundled данные до завершения выбора remote/fallback; запрос каталога ограничен `8` секундами, отмена завершается тихо и освобождает wrapper, а фоновые обновления сохраняют видимый контент.
