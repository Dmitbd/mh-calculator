...
- Данные: удалённые комплекты проходят версионную runtime-валидацию до попадания в экран; повреждённый ответ не заменяет локальный fallback.
...
- Данные: загружаемые draft/published payload проверяются по тому же доменному контракту, что и пользовательские билды.
