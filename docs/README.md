...
- Интерфейс: шапка называется `Призыв`, под инструкцией показан заголовок `Калькулятор соперничества за призыв героев`; порядок блоков повторяет калькулятор антиквариата, очки показаны одной компактной строкой, а затраты — числом после иконки алмаза сверху секции `Покупки за алмазы`.
