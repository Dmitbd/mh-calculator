...
- Данные: один ограниченный `/bootstrap` через публичную anon/RLS-границу подтверждает backend и совместимость полного пагинированного manifest до чтения опубликованных билдов; удалённые комплекты проходят ограниченную resource budgets версионную runtime-валидацию data properties, а общая машина источников при сбое отдельного ресурса заменяет только его текущим bundled fallback.
