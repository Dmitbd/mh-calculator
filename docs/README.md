...
- Данные: один ограниченный `/bootstrap` подтверждает backend и совместимость manifest до чтения опубликованных билдов; удалённые комплекты проходят ограниченную resource budgets версионную runtime-валидацию data properties, а сбой отдельного ресурса заменяет только его текущим bundled fallback.
