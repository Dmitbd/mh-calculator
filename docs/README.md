...
- Тип: пользовательская функция с действиями редактирования и удаления только для подтверждённого администратора.
- Spec: [hero-builds-spec.md](hero-builds-spec.md).

### Билдер билдов

- Точка входа: `/admin/branch-builder`.
- Назначение: создание, сохранение черновиков, проверка, публикация, загрузка, удаление и JSON-экспорт комплектов билдов героя.
- Тип: административная функция, защищённая Supabase admin claim и RLS.
