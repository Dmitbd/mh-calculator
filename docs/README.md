...
- Назначение: создание, сохранение черновиков, проверка, атомарная публикация, редактирование опубликованного комплекта по локальным draft-вкладкам, загрузка и JSON-экспорт билдов героя.
- Данные: загружаемые draft/published payload проверяются по тому же доменному контракту, что и пользовательские билды; server-сохранения защищены revision-конфликтами и оставляют immutable историю опубликованных снимков; неуспешный переход к другому герою сохраняет принятую форму редактируемой.
