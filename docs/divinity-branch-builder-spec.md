...
Публичные viewer-экраны допускают чтение published resource только после совместимого `/bootstrap`. Этот gate не заменяет admin authentication/RLS и намеренно не оборачивает lifecycle-записи билдера: административные операции продолжают использовать собственную авторизованную repository-границу, revision и conflict semantics. Edge Function использует публичный `SUPABASE_ANON_KEY`, читает только разрешённые RLS опубликованные строки и возвращает детерминированный manifest без service-role bypass или секретов в клиенте.
