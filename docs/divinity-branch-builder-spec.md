...
- Успешный выход из `mode=edit` инвалидирует identity загруженной формы, активного героя, revision и pending requests. После следующего входа опубликованная строка обязательно загружается заново и формирует чистый baseline с актуальной revision; отклонённый выход не сбрасывает локальные поля.
