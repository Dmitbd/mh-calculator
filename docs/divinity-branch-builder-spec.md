...
Публичные viewer-экраны допускают чтение published resource только после совместимого `/bootstrap`. Этот gate не заменяет admin authentication/RLS и намеренно не оборачивает lifecycle-записи билдера: административные операции продолжают использовать собственную авторизованную repository-границу, revision и conflict semantics. Edge Function читает service-role key только в server runtime и возвращает лишь детерминированный manifest опубликованных данных без секретов.
